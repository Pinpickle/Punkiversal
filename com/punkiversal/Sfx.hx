...
class Sfx
{
	...
	public function play(volume:Float = 1, pan:Float = 0, loop:Bool = false)
	{
		...
		if (playing) stop();
		_pan = PV.clamp(pan, -1, 1);
		_volume = volume < 0 ? 0 : volume;
		_filteredPan = PV.clamp(_pan + getPan(_type), -1, 1);
	...
	{
		value = PV.clamp(value, -1, 1);
		if (_channel == null || _pan == value) return value;
		var filteredPan:Float = PV.clamp(value + getPan(_type), -1, 1);
	...
	static public function setPan(type:String, pan:Float)
	{
		...
		transform.pan = PV.clamp(pan, -1, 1);
